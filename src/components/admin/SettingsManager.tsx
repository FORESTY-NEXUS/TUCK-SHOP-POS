...
export default function SettingsManager({
  ...
  const [printingEnabled, setPrintingEnabled] = useState(initialSettings.printingEnabled ?? true);
  ...
  return (
    <div className="space-y-6">
      ...
      <Section icon={Printer} title="Thermal Printer" subtitle="Customer receipts">
        <div className="flex items-center gap-3 max-w-md">
          <input type="checkbox" id="printingEnabled" checked={printingEnabled} onChange={(e) => setPrintingEnabled(e.target.checked)} className="w-5 h-5 text-brand-600 border-stone-300 rounded focus:ring-brand-500" />
          <div>
            <label htmlFor="printingEnabled" className="text-sm font-medium text-stone-900 cursor-pointer">Print receipts automatically</label>
            <p className="text-xs text-stone-500 mt-0.5">Turn off to stop the silent thermal print on every sale — sales still save either way.</p>
          </div>
        </div>
        <div>
          <label className="text-xs font-medium text-stone-700 block mb-1">Windows Printer Name</label>
          <input type="text" value={printerName} onChange={(e) => setPrinterName(e.target.value)} placeholder="e.g., EPSON TM-T20II Receipt" disabled={!printingEnabled} className="input max-w-md disabled:opacity-50" />
          <p className="text-xs text-stone-500 mt-1.5">
            Exact queue name from Windows "Printers & Scanners". Leave blank to disable printing — sales still save.
          </p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-md">
          <div>
            <label className="text-xs font-medium text-stone-700 block mb-1">Printer Type</label>
            <select value={printerType} onChange={(e) => setPrinterType(e.target.value)} disabled={!printingEnabled} className="input disabled:opacity-50">
          ...
          <div>
            ...
            <input type="number" min={1} value={paperWidth} onChange={(e) => setPaperWidth(e.target.value)} disabled={!printingEnabled} className="input disabled:opacity-50" />
            <p className="text-xs text-stone-500 mt-1">32/48 = 58mm · 48/64 = 80mm</p>
          </div>
        </div>
        <div className="flex items-center gap-2 max-w-md">
          <input type="checkbox" id="autoCut" checked={autoCut} onChange={(e) => setAutoCut(e.target.checked)} disabled={!printingEnabled} className="w-4 h-4 text-brand-600 border-stone-300 rounded focus:ring-brand-500" />
