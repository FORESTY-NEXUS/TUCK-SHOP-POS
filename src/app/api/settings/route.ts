...
export async function PATCH(req: NextRequest) {
  ...
  const {
    rupeesPerPoint, printerName, printerType, paperWidth, autoCut,
    speedDial, autoFillDeliveryAddress,
  ...
  // Validate autoFillDeliveryAddress
  if (autoFillDeliveryAddress !== undefined && typeof autoFillDeliveryAddress !== "boolean") {
    return NextResponse.json(
      { error: "autoFillDeliveryAddress must be a boolean" },
      { status: 400 }
    );
  }
  ...
  if (autoFillDeliveryAddress !== undefined) update.autoFillDeliveryAddress = autoFillDeliveryAddress;
