  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [reportRes, historyRes, customersRes] = await Promise.all([
        ...
        fetch(`/api/orders/history?${qs.toString()}&page=1`),
        fetch("/api/customers"),
      ]);
                  ...
                  <tbody className="divide-y divide-stone-100">
                    {recent.map((o: any) => (
                      <tr key={o._id} className="hover:bg-stone-50">
                        <td className="px-4 py-2.5 font-medium text-brand-700 tabular">{o.orderNumber}</td>
                        <td className="px-4 py-2.5">{o.customer?.name || o.customer?.phone || "Guest"}</td>
                        <td className="px-4 py-2.5 capitalize">{o.type.replace("_", " ")}</td>
                        ...
                        <td className="px-4 py-2.5 text-center">
                          <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                            ...
                            o.status === "cancelled" ? "bg-stone-200 text-stone-600" : "bg-amber-100 text-amber-800"
                          }`}>
